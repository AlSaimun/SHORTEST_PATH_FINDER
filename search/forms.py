...
class ShortestDistanceForm(forms.Form):
    source_district = forms.ModelChoiceField(queryset=District.districts.all(), widget=forms.Select(attrs={'class': 'form-select col-6'}))
    destination_district = forms.ModelChoiceField(queryset=District.districts.all(),widget=forms.Select(attrs={'class': 'form-select col-6'}))
    def clean(self):
